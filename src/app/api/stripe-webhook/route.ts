  ...
  await prisma.userSubscription.upsert({
    ...
    create: {
      ...
      trialEnd: subscription.trial_end
        ? new Date(subscription.trial_end * 1000)
        : null,
      hasTrialExpired: subscription.trial_end ? now > new Date(subscription.trial_end * 1000) : false,
    },
    update: {
      stripePriceId: subscription.items.data[0].price.id,
      stripeCurrentPeriodEnd: new Date(subscription.current_period_end * 1000),
...
function determineSubscriptionStatus(subscription: Stripe.Subscription): 'free' | 'trial' | 'premium' | 'premium_plus' {
  ...
  if (subscription.status === 'active') {
    ...
    if (subscription.items.data[0].price.id === env.NEXT_PUBLIC_STRIPE_PRICE_ID_PRO_MONTHLY) {
      return 'premium';
    } else if (subscription.items.data[0].price.id === env.NEXT_PUBLIC_STRIPE_PRICE_ID_PRO_PLUS_MONTHLY) {
      return 'premium_plus';
    }
  }
